...
本周AWS重点新闻包括：1) AWS re:Inforce安全会议发布多项安全创新，包括IAM Access Analyzer增强功能、根用户MFA强制执行、AWS Network Firewall威胁情报集成等；2) Amazon Verified Permissions团队发布Express.js开源包，简化Web应用API的外部细粒度授权实现；3) AWS Lambda现在原生支持Avro和Protobuf格式的Kafka事件；4) Amazon S3 Express One Zone推出对象原子重命名API；5) Valkey发布GLIDE 2.0版本，支持Go语言、OpenTelemetry和管道批处理；6) AWS即将在全球多个城市举办AWS Summits和GenAI Lofts活动，包括日本、印度、纽约等地。
