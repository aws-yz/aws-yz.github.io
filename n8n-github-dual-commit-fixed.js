// n8n GitHub 双重提交节点代码 - 修复版本
// 修复凭据访问问题

const inputData = $json;

// 方法1：如果你在Code节点中配置了GitHub凭据，使用这种方式
// const githubToken = $credentials.githubApi.accessToken;

// 方法2：推荐使用HTTP Request节点而不是Code节点来调用GitHub API
// 但如果必须在Code节点中使用，需要通过环境变量或其他方式传递token

// 临时解决方案：使用HTTP Request节点的方式
// 这个代码应该在HTTP Request节点中使用，而不是Code节点
...
// 如果你必须在Code节点中使用，请确保：
// 1. 在Code节点的设置中添加了GitHub凭据
// 2. 或者通过其他方式传递token

// 检查是否有token（从前一个节点传递或环境变量）
let githubToken;
try {
  // 尝试从凭据获取（需要在节点设置中配置）
  ...
  return {
    status: 'error',
    error: 'GitHub token not available. Please configure GitHub credentials in the node settings.',
...
if (!githubToken) {
  return {
    status: 'error',
    error: 'GitHub token is empty. Please check your credentials configuration.',
    timestamp: new Date().toISOString(),
    fileName: inputData.fileName,
    title: inputData.title,
    articleCommit: null,
